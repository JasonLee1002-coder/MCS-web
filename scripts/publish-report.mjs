/**
 * publish-report.mjs — 各長官通用報告發布工具
 *
 * 用法：
 *   node scripts/publish-report.mjs <html_file> <slug> [category] [author]
 *
 * 範例：
 *   node scripts/publish-report.mjs "C:/path/to/報告.html" david-closure-20260529 closure CLO
 *   node scripts/publish-report.mjs "C:/path/to/分析.html" lexforge-compliance-20260529 analysis CLO
 *
 * category: report | closure | analysis | meeting
 * 發布後可在 https://poc.mcstation.ai/poc/<slug> 存取
 */
import { readFileSync } from 'fs'
import { resolve } from 'path'

const POC_PASSWORD = 'xu4bj6D1l41le4'
const BASE_URL = 'https://poc.mcstation.ai'

const [,, htmlFile, slug, category = 'report', author = 'CLO'] = process.argv

if (!htmlFile || !slug) {
  console.error('用法: node publish-report.mjs <html_file> <slug> [category] [author]')
  process.exit(1)
}

// 讀取 HTML
const htmlContent = readFileSync(resolve(htmlFile), 'utf-8')

// 從 HTML <title> 抓標題
const titleMatch = htmlContent.match(/<title[^>]*>([^<]+)<\/title>/i)
const title = titleMatch?.[1]?.trim() ?? slug

// 發布
const res = await fetch(`${BASE_URL}/api/poc`, {
  method: 'POST',
  headers: { 'Content-Type': 'application/json', 'x-poc-token': POC_PASSWORD },
  body: JSON.stringify({ title, category, author, htmlContent, slug }),
})

if (res.ok) {
  const { id } = await res.json()
  console.log(`✅ 報告已發布`)
  console.log(`   標題：${title}`)
  console.log(`   網址：${BASE_URL}/poc/${slug}`)
  console.log(`   ID：${id}`)
} else {
  const err = await res.json().catch(() => ({}))
  console.error('❌ 發布失敗：', JSON.stringify(err))
  process.exit(1)
}
