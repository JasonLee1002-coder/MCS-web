...
const RULES = [
  ...
  {
    level: 'BLOCK',
    name: '真實企業/機構＋評分或指標（無單位）',
    // 2026-08-26 立。上一條規則要求數字後面接 %／倍／成，所以
    // 「4.89 ★ 東方美加盟滿意度評分」這種**沒有單位的評分**整條溜過去，
    // 而它就掛在首頁最上方的統計卡片上。
    // 判準跟上一條一樣：把指標掛在具名真實企業身上，就要有可回查來源。
    // 白名單登記「東方美是客戶」不等於授權「東方美滿意度 4.89」——
    // 見 verified-claims.md 使用規則第 3 條。
    // 三語系都要涵蓋：中文「滿意度」、日文「満足度」、英文 Satisfaction／NPS。
    // 英文版寫的是 "EB+ Franchise Satisfaction Score"（EB+ 是東方美的英文代號），
    // 只比對中文公司名會漏掉，所以英文分支改用指標詞本身當錨點。
    re: /(麥味登|東方美|麗嬰|Acer|XMART|鼎新|中華電信|統一超商|首都高|EB\+)[^。\n]{0,30}?(滿意度|満足度|評分|分數|スコア|好評|回購率|NPS|留存)|((滿意度|満足度|評分|分數|回購率|NPS)[^。\n]{0,20}?(麥味登|東方美|麗嬰|Acer|XMART|鼎新))|(Franchise|Customer|Partner)\s+(Satisfaction|NPS|Retention)\s*(Score|Rate)?/i,
    hint: '有問卷／後台數據等可回查來源就加白名單並標明期間與樣本數；沒有就改成不帶數字的敘述',
  },
  {
    level: 'BLOCK',
    name: '杜撰引言（引號＋真實人名署名）',
